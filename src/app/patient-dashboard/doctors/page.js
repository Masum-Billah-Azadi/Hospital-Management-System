  ...
  const fetchDoctors = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/doctors?designation=${encodeURIComponent(selectedDesignation)}&name=${encodeURIComponent(searchTerm)}`,
      );
    ...
    <div className="flex flex-col gap-6">
      <Card className="w-full bg-light-card dark:bg-dark-card">
        <CardBody>
          <Typography
            variant="h5"
            className="mb-4 text-light-text-primary dark:text-dark-text-primary"
          >
            Find Your Doctor
          </Typography>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Menu>
              ...
              <MenuList className="w-full md:w-72 bg-light-card dark:bg-dark-card text-light-text-primary dark:text-dark-text-primary border-gray-300 dark:border-gray-700">
                <MenuItem onClick={() => setSelectedDesignation("")}>
                  All Specialties
                </MenuItem>
                {specialties.map((specialty) => (
                  <MenuItem
                    key={specialty}
                    onClick={() => setSelectedDesignation(specialty)}
                  >
                    {specialty}
                  </MenuItem>
                ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {doctorProfiles.length > 0 ? (
              doctorProfiles.map((profile) => (
                <Card
                  key={profile._id}
                  className="flex flex-col text-center bg-light-card dark:bg-dark-card text-light-text-primary dark:text-dark-text-primary"
                >
                  <CardBody className="flex flex-col items-center flex-1 p-4">
                    <Avatar
                      src={
                        profile.user.image ||
                        `https://ui-avatars.com/api/?name=${profile.user.name.replace(/\s/g, "+")}&background=random`
                      }
                  ...
                  <CardFooter className="pt-0 p-4">
                    <Link
                      href={`/patient-dashboard/doctor-profile/${profile.user._id}`}
                    >
                      <Button
                        variant="outlined"
                        color="blue"
                    ...
                    {/* --- নতুন লজিক: অ্যাভেলেবেল থাকলে বুকিং বাটন, না থাকলে Not Available --- */}
                    {profile.isAvailable === false ? (
                      ...
                      <Link
                        href={`/patient-dashboard/book-appointment/${profile.user._id}`}
                      >
              ...
              <div className="col-span-full text-center py-10">
                <Typography className="text-light-text-secondary dark:text-dark-text-secondary">
                  No doctors found.
                </Typography>
              </div>
            )}
