...
const makeMedicinesHtml = (meds) => {
  ...
  return meds.map((med, i) => {
    const duration = med.duration ? `${med.duration.value || ''} ${med.duration.unit || ''}` : '';
    const safe = (v) => (v !== undefined && v !== null ? String(v) : '');
    return `
      <tr>
        <td style="padding:8px;border-bottom:1px solid #f0f0f0;">
          <strong>${safe(med.medicationName)}</strong>
          ${med.dosage ? `<div style="color:#666;font-size:12px;margin-top:4px;">${safe(med.dosage)}</div>` : ''}
        </td>
        <td style="padding:8px;border-bottom:1px solid #f0f0f0;">${safe(med.frequency)}</td>
        <td style="padding:8px;border-bottom:1px solid #f0f0f0;">${safe(med.instruction)}</td>
        <td style="padding:8px;border-bottom:1px solid #f0f0f0;">${duration}</td>
        <td style="padding:8px;border-bottom:1px solid #f0f0f0;">${med.price ? `৳${safe(med.price)}` : ''}</td>
      </tr>
    `;
  }).join('');
};
...
const getHtmlTemplate = (prescription, patient, doctor, qrCodeDataURL) => {
  ...
  // Follow-up display:
  let followUpHtml = '';
  if (prescription.followUp && (prescription.followUp.value || prescription.followUp.unit)) {
    const unitMap = { day: 'day(s)', week: 'week(s)', month: 'month(s)', continue: 'Continue' };
    const value = prescription.followUp.value || '';
    const unitStr = unitMap[prescription.followUp.unit] || prescription.followUp.unit || '';
    followUpHtml = `<p><strong>Follow-Up:</strong> ${value ? `${value} ${unitStr}` : unitStr}</p>`;
  }
...
export async function GET(request, { params }) {
  try {
    ...
    const patient = await PatientProfile.findById(prescription.patientProfile).populate('user', '_id name phone').lean();
